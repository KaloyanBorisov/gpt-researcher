...
from services.export_service.main import export_document, ExportRequest, ExportFormat
...
async def main(args):
    ...
    selected_tone = tone_map.get(args.tone.lower(), Tone.Objective)
    ...
    if not args.no_pdf:
        try:
            exp_res = await export_document(ExportRequest(
                report_markdown=report,
                title=task_id,
                format=ExportFormat.pdf
            ))
            print(f"PDF written to '{exp_res.file_path}'")
    ...
    if not args.no_docx:
        try:
            ...
            print(f"DOCX written to '{exp_res.file_path}'")
        except Exception as e:
